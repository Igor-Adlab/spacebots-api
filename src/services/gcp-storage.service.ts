...
export class GcpStorageService {
  ...
  constructor({ redis, logger }) {
    ...
    if (env.RUN_GCP_STORAGE_CLEANER) {
      this.logger.info('Cleaner started!');
      new Worker(STORAGE_CLEANER_QUEUE, this.processor.bind(this), {
        concurrency: 10,
        connection: redis,
      });
    }
  ...
  private async processor(job: Job<RemovePayload>) {
    return this.remove(job.data);
  }
